...
Supplemental Table 4 File aay9189_TableS4.xlsx, first worksheet, save as text 
file TableS4.txt (the Excel file will not read cleanly into R).
...
```{r}
mat_df = read_tsv('S1_filtered_cpm.tsv')
mat = mat_df[,-1]
...
```{r}
unitn = read_excel(
  'microbial_databases/UNITN/1-s2.0-S0092867419300017-mmc4.xlsx', sheet=1) %>%
  clean_names() %>%
...
Reading the WIS data.

```{r}
wis = read_tsv('microbial_databases/WIS/TableS4.txt', show_col_types = FALSE) %>%
  clean_names()
```
Note that read_tsv *correctly* notes a problem with row 3
column 9 (all other data in the column is blank, so read_tsv cannot determine the
data type).  This problem does not affect downstream analysis.

```{r}
problems(wis)
```

Consider those genera in the WIS set that could possibly match an infected sample
...
by the curators of the WIS data.

```{r}
passing_filters = function(cand) {
  critical_cols = seq(14,74,5)
  passing = rep(FALSE, nrow(cand))
  for (col in critical_cols) {
    these_passing = cand[[col]] == 1 | cand[[col]] == 'NR'
