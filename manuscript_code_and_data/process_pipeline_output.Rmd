...
```{r}
total_reads <- read_tsv("total_reads.tsv") %>%
  select(-index_files) %>%
  deframe()
```

Load the genus x sample read count data and remove any reads mapped to human.
...
Every row (genus) has at least one count.
```{r}
