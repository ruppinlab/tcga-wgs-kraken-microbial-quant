...
def main():
    ...
    parser.add_argument(
        "-i", "--input", dest="in_file", required=True, help="Input kraken report file."
    )
    parser.add_argument(
        "-k",
        "--kmer_distr",
    ...
    for line in i_file:
        ...
        # Add node to tree
        curr_node = Tree(
            name, taxid, level_num, level_id, all_reads, level_reads, None, prev_node
        )
    ...
    for line in k_file.readlines()[1:]:
        [mapped_taxid, mapped_taxid_dict] = process_kmer_distribution(
            line, lvl_taxids, map2lvl_taxids
        )
    ...
    while len(curr_nodes) > 0:
        ...
        for child_node in sorted(
            curr_node.children, key=operator.attrgetter("all_reads")
        ):
            # Add if at level or above
            if (
                child_node.level_id[0] != args.level
                or child_node.level_id == args.level
            ):
        ...
        if curr_node.taxid in new_reads:
            ...
            r_file.write(
                "%0.2f\t" % (float(new_all_reads) / float(sum_all_reads) * 100)
            )
